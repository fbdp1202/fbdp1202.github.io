...
이러한 점에서 이제 **K-Nearest Neighbor(KNN)** 에 대해 다룰 것이다.

![](/assets/img/dev/mldl/cs231n/lecture02/cs231n-02-003-Nearest_Neighbor_ex.png)

<strong><span style="color:red"> K-Nearest Neighbor(KNN) </span></strong> 은 **가까운 이웃을 K**개 만큼 찾고, **이웃끼리 투표**를 하는 방법이다.

아래 그림을 보면 K 값이 증가함에 따라서 **경계가 점점 부드러워 지고 있는 것을 볼 수 있다.**

이러한 방식을 이용하면 좀 더 일반화 된 **결정 경계**를 찾을 수 있다.

여기서 K값의 증가함에 따라서 부드러워 지지만, **흰색 영역이 증가** 하는 것을 볼 수 있다.

이 **흰색 영역** 은 어느 쪽에도 분류 할지 **알 수 없는 영역**이다.
...
우리는 아직까지 **어떻게 가까운 이웃을 찾아내는지** 알지 못한다.

이 KNN 알고리즘에서 가까운 이웃을 찾아내는 방법으로 아래와 같은 <strong><span style="color:red">Distance Matrix</span></strong>를 사용한다

<strong><span style="color:red">Distance Matrix</span></strong>은 데이터 간에 거리를 측정하는 방법으로 크게 **L1과 L2 방식**으로 나누어진다.

**L1**은 `각 성분의 차이`를 모두 더한 것이며, **L2**는 `유클리드 거리`를 사용한다.

이 두가지 방법에 차이를 음미하면 다음과 같다.

- **L1의 특징**
  + 기존의 좌표계를 **회전하면 거리가 바뀐다.**
  + **각 성분이 개별적인 의미** 를 가지고 있을 때 L1이 어울린다.
  + 예시) 키, 몸무게

- **L2의 특징**
  + 위와 다르게 회전에도 거리가 변하지 않는다.
  + 일반적인 벡터이며, 요소들간의 실직적인 의미가 없는 경우 사용

- **L1과 L2의 결정**
  + 사실적으로 데이터의 성분 관계를 정확하게 알지 못하는 경우가 대부분이다.
  + 이를 정하는 것은 깊게 생각하지 말고, **둘다 사용해보고 성능이 더 좋은 것을 사** 하자.
...
자 위에서 배운 KNN을 보면 `K, Distance Matrix`와 같이 **우리가 직접 정해 주어야 하는 값** 들이 있을 것이다.

이처럼 우리가 직접 정해주어야 하는 값을 <strong><span style="color:red">Hyperparameter(초모수)</span></strong> 라고 한다.

이러한 **Hyperparameter의 값은 상황에 따라서 다른 값을 가지므로** 이를 찾아가는 과정이 필요하다.

이러한 Hyperparameter를 찾아가는 일반적인 방법을 소개하고자 한다.
...
자 우리에게 어떤 데이터셋(Dataset)이 주어졌다고 생각하자.

아래 그림에서는 이러한 **데이터셋을 사용하는 방법 3가지** 를 소개한다. 한번 살펴보자
...
- Idea #2: 데이터를 **Train과 Test** 로 나누는 방법
  + 그러면 Train과 Test로 나누어서 사용하면 되는거 아닌가?
  + K값을 바꾸어 가면서 Test 중에 가장 높은 성능을 가지는 K값을 찾으면 되겠다!
...
- Idea #3: 데이터를 **Train, Validation(Dev), Test** 로 나누는 방법
  + 이제 우리는 validation를 추가로 사용할 것이다.
  + 이제 **Train를 한 뒤에 이 Validation를 이용하여 좋은 성능을 가지는 Hyperparameter를 찾을 것** 이다.
  + 이 Validation에서 **좋은 성능을 가지는 Hyperparameter를 찾은 뒤** 에 **마지막에 딱 한번 Test 데이터를 테스트** 하여, 이 데이터를 최대한 **"Unseen Data"로 활용하는 방법** 이다.
  + 위 방법들 중 <strong><span style="color:red">이 방법이 제일 올바른 방법</span></strong> 이다.
<br>
...
위에서 Hyperparameter를 찾는 3가지 방법을 배웠다.

위 3가지 방법과 별개로 **또 다른 방법**인 `Cross-Validation`를 소개하고자 한다.

Cross-Validation 은 아래 그림과 같이 **Dataset를 Fold 단위로 자르고**, 이 **fold 중 하나를 validation으로 선택하고 나머지를 train 데이터로 사용**한다.

위 방법으로 **Validation으로 사용할 fold를 바꿔가면서 반복**하고, 이중에 **가장 좋은 성능을 가지는 Hyperparameter를 찾아내는 방법**이다.

이 방법은 **validation의 데이터가 편향되는 현상**을 방지할 수 있다.
...
이 예시는 5-fold cross-validation를 이용하여 **학습한 결과**이다.

아래 왼쪽 그래프는 `가로축은 K값`을, `세로축은 성능`을 나타낸다.

성능은 `K값이 7`일때 가장 좋은 성능을 보이는 것을 볼 수 있다.

하지만 이것이 **항상 K가 7일때 좋은 결과를 가진다는 것**이 아니다.

`데이터셋의 종류, 찾는 문제`에 따라서 이것은 **다시 구해져야한다.**
...
하지만 지금까지 열심히 공부한 **Nearest Neighbor** 은 **이미지에서 절대로 사용되지 않는다.**

우선 초반에 언급한 **predict과정이 오래 걸린다는 것**과, <strong><span style="color:red">사진 간에 distance 값이 그렇게 의미있는 값이 아니기 때문</span></strong>이다.

아래 예시를 보자

아래에는 가장 왼쪽에 원본 이미지와 변형된 3개의 이미지를 보여준다.

여기서 재밋는 부분은 **원본사진과 각각의 사진에 거리가 모두 같은 사진**이다.

이러한 관점에서 이미지의 Distance의 값은 그렇게 의미 있는 값이 아니다.
...
**Curse of dimension(차원의 저주)** 란 KNN이 잘 작동하기 위해서는 **전체 공간을 조밀하게 커버할 수 있을 정도의 데이터가 필요하다** 는 이야기이다.

여기서 Curse of dimension 이란 아래 그림과 같다.
...
이와 같이 **고차원으로 갈 수록 기하급수 적인 데이터가 필요** 하다.

이 필요한 데이터가 4개라면 괜찮지만 10000개가 필요하다고 하면 실감할 수 있을 것이다.
...
자 이제 <strong><span style="color:red">Linear Classification(선형 분류)</span></strong>라는 것을 알아보자.

**Linear Classification** 은 단순하지만 이후에 배우게 되는 **Neural Network**와 **CNN**의 기반이 되는 알고리즘이다. 즉 아래 그림과 같이 `기본 블럭`이 되는 것이다.
...
Parametric model에는 `두가지 성분`이 있다.
...
아래 그림에서는 기본적인 Linear Classification 예시를 보여준다.
...
아래는 이해를 돕기위한 Linear Classification의 한 예시이다.

아래는 **입력 이미지의 크기가 2x2=4** 라고 가정했다.

분류할 카테고리는 **3가지 (고양이/개/양)** 이라고 하자.

각 성분의 크기는 다음과 같다

1. `X`: 입력 이미지
    + `X의 크기: 4x1`로 1차원 으로 펴진다.

2. `W`: 가중치 값(Weight Parameter)
    + `W의 크기: 3x4`로 **(클래스 갯수) x (입력 데이터 크기)** 이다.
    + **각 행(row)은 하나의 클래스를 담당** 하므로 이를 <strong><span style="color:red">템플릿</span></strong>으로 볼 수 있다.
...
아래 결과는 CIFAR-10 이라는 데이터셋을 이용하여 Linear Classification를 학습한 결과이다.

가장 아래 보이는 10개의 그림은 **각각 템플릿의 weight 값**을 나타낸 것이다.

이 weight 값은 각 class에 있는 모든 이미지를 평균화 시키므로 **다양한 모습의 사진이 있지만 하나의 템플릿 값만 가진다.**

아래의 Car 사진을 보면 진짜 자동차와 같은 모습을 볼 수 있다.

하지만 말을 보자. 말은 왼쪽 모습과 오른쪽 모두 있을 수 있어 이것이 겹쳐서 보인다.
...
또 다른 관점은 이미지를 고차원에서 하나의 점으로 생각하는 것이다.

이러한 관점으로 Linear Classification을 해석하면 아래 그림과 같다.

1. `W`: 가중치 값 - **각 선분의 기울기**
2. `b`: Bias 값 - **선분의 시작 offset 값**

이러한 관점에서 **각 class은 선분을 가지며 이러한 선을 기준으로 분류** 하는 것으로 해석할 수 있다.
...
이러한 Linear Classification으로 풀 수 없는 어려운 경우가 존재한다.

아래 그림은 Linear Classification으로 분류할 수 없는 경우를 보여준다.

각각은 `Partity Problem(Xor)`, `도넛 형태`, `Multimodal Problem` 이다

안타깝지만, 이러한 경우 Linear Classification 으로 풀 수 없는 문제이다.
...
결국 Linear Classification은 **단순히 행렬과 벡터의 곱** 의 형태라는 것을 알았고, **템플릿 매칭** 과 관련이 있다.

이러한 관점에서 **각 카테고리에 대해 하나의 템플릿을 학습** 한다는 것도 배웠다.

그리고 가중치 행렬 W를 학습시키고 나면 새로운 학습 데이터에도 스코어를 매길 수 있다.

이러한 관점에서 **Train 시간** 은 `O(N)` 만큼 필요하지만 **Test 시간** 은 `O(1)`으로 우리가 원하던 Test 시간을 가진다.
